...
"""
# TODO: write an algorithm that recursively saves, get_or_create for specific ones
# Application, ModelMirror, ModelField
# should this be done via overwriting save?
    ...
    def emit(self, record):
        ...
        try:
            ...
            if not hasattr(record, 'action'):
                ...
                if signal:
                    ...
                    if self.maxage:
                        ...
                        Unspecified.objects.filter(
                            created_at__lte=current - self.maxage
                        ).delete()
            ...
            if record.action == 'model':
                if (
                    'al_evt' in record.data['instance'].__dict__.keys()
                    and record.data['instance'].al_evt
                ):
                ...
                if (
                    'al_chl' in record.data['instance'].__dict__.keys()
                    and record.data['instance'].al_chl
                    and not entry.modification
                ):
