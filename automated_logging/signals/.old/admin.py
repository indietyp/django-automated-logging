...
class ModelAdmin(ReadOnlyAdminMixin):
    ...
    def get_prev(self, obj):
        if obj.modification is not None and obj.modification.modification is not None:
            return "\n ".join(
                str(v) for v in obj.modification.modification.previously.all()
            )
        else:
            return ''
    ...
    def get_curr(self, obj):
        if obj.modification is not None and obj.modification.modification is not None:
            return ", ".join(
                str(v) for v in obj.modification.modification.currently.all()
            )
        else:
            return ''
    ...
    list_filter = ('updated_at', 'action', UserActionListFilter, ContentTypeListFilter)
...
class RequestAdmin(ReadOnlyAdminMixin):
    ...
    list_display = ('user', 'method', 'uri', 'application', 'updated_at')
...
class UnspecifiedAdmin(ReadOnlyAdminMixin):
    ...
    list_display = ('level', 'file', 'line', 'message', 'updated_at')
